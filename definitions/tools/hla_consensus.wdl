...
task hlaConsensus {
  ...
  command <<<
    ...
    if clinical_exists:
        ...
        raw_clinical_i_calls = ["~{sep="\",\"" select_first([clinical_mhc_classI_alleles, []])}"]
        #MHC Class II clinical typing results
        raw_clinical_ii_calls = ["~{sep="\",\"" select_first([clinical_mhc_classII_alleles, []])}"]
