...
task alignedSeqFdaStats {
  ...
  String output_file = output_name + "_aligned_metrics" + suffix + ".txt"
  command <<<
    /usr/bin/perl - ~{reference} ~{sep=" " files} > "~{output_file}" <<'EOF'
        #!/usr/bin/perl
...
EOF
  ...
  output {
    File aligned_stats = "~{output_file}"
  }
}
